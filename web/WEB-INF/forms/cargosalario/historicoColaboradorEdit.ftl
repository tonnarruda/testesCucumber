...
<body>
	...
	<@ww.form name="form" action="${formAction}" onsubmit="enviaForm();"  validate="true" method="POST">
		...
		<@authz.authorize ifAllGranted="ROLE_COMPROU_SESMT">
			...
			<@ww.select label="Função" name="historicoColaborador.funcao.id" id="funcao" required="${obrigarAmbienteFuncao?string}" list="funcaos" listKey="id" listValue="nome" headerValue="Nenhum" headerKey="-1" />
		</@authz.authorize>
		<@authz.authorize ifNotGranted="ROLE_COMPROU_SESMT">
