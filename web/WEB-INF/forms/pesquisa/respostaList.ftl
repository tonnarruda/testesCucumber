...
<body>
	...
	<@display.table name="respostas" id="resposta" class="dados" sort="list">
		<@display.column title="Ações" class="acao">
			...
			<a href="#" onclick="if (confirm('Confirma exclusão?')) window.location='delete.action?resposta.id=${resposta.id}&pergunta.id=${pergunta.id}&pesquisa.id=${pesquisa.id}'"><img border="0" title="<@ww.text name="list.del.hint"/>" src="<@ww.url includeParams="none" value="/imgs/delete.gif"/>"></a>
		</@display.column>
