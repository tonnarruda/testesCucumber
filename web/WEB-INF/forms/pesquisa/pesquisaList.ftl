	...
	<@display.table name="pesquisas" id="pesquisa" class="dados">
		<@display.column title="Ações" style="width:210px">
			<#if !pesquisa.questionario.liberado>
				<a href="javascript:if (confirm('Deseja liberar esta pesquisa?')) window.location='../questionario/liberar.action?questionario.id=${pesquisa.questionario.id}'"><img border="0" title="Liberar Pesquisa" src="<@ww.url includeParams="none" value="/imgs/liberar.gif"/>"></a>
			<#else>
			...
			<a href="javascript:if (confirm('Confirma exclusão?')) window.location='delete.action?pesquisa.id=${pesquisa.id}&pesquisa.questionario.empresa.id=${pesquisa.questionario.empresa.id}&page=${page}'"><img border="0" title="<@ww.text name="list.del.hint"/>" src="<@ww.url includeParams="none" value="/imgs/delete.gif"/>"></a>
