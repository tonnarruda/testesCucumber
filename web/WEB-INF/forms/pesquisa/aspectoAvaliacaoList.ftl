...
<body>
	...
	<@display.table name="aspectos" id="aspecto" class="dados">
		<@display.column title="Ações" class="acao">
			<a href="prepareUpdate.action?aspecto.id=${aspecto.id}&avaliacao.id=${avaliacao.id}&modeloAvaliacao=${modeloAvaliacao}"><img border="0" title="Editar" src="<@ww.url value="/imgs/edit.gif"/>"></a>
			<a href="#" onclick="newConfirm('Confirma exclusão?', function(){window.location='deleteAvaliacao.action?aspecto.id=${aspecto.id}&avaliacao.id=${avaliacao.id}&modeloAvaliacao=${modeloAvaliacao}'});"><img border="0" title="Excluir" src="<@ww.url value="/imgs/delete.gif"/>"></a>
		</@display.column>
