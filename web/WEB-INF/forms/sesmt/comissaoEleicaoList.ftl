...
<body>
	...
	<div id="box">
		...
		<@ww.form name="form" id="form" action="insert.action" onsubmit="${validarCampos}" validate="true" method="POST">
			<@frt.checkListBox label="Colaboradores" name="colaboradorsCheck" list="colaboradorsCheckList" form="document.getElementById('form')"/>
	...
	<@ww.form name="formList" id="formList" action="saveFuncao.action" method="POST">
		<@display.table name="comissaoEleicaos" id="comissaoEleicao" class="dados">
			<@display.column title="Ações" class="acao" style="width: 40px;">
				<a href="#" onclick="if (confirm('Confirma exclusão?')) window.location='delete.action?comissaoEleicao.id=${comissaoEleicao.id}&eleicao.id=${eleicao.id}'"><img border="0" title="Excluir" src="<@ww.url value="/imgs/delete.gif"/>"></a>
			</@display.column>
