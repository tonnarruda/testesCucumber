...
<body>
	...
	<@display.table name="pcmats" id="pcmat" class="dados">
		<@display.column title="Ações" class="acao">
			<a href="javascript: executeLink('gerar.action?pcmat.id=${pcmat.id}&obra.id=${obra.id}');"><img border="0" title="Gerar Documento" src="<@ww.url value="/imgs/ico_file_word.png"/>"></a>
			<a href="javascript: executeLink('prepareUpdate.action?pcmat.id=${pcmat.id}');"><img border="0" title="Editar" src="<@ww.url value="/imgs/edit.gif"/>"></a>
			<#if ultimoPcmat?exists && ultimoPcmat.id ==  pcmat.id>
				...
				<a href="javascript:;" onclick="javascript:newConfirm('Confirma exclusão?', function(){executeLink('delete.action?pcmat.id=${pcmat.id}&obra.id=${obra.id}');});"><img border="0" title="<@ww.text name="list.del.hint"/>" src="<@ww.url includeParams="none" value="/imgs/delete.gif"/>"></a>
			<#else>
	...
	<div class="buttonGroup">
		<button class="btnInserir" onclick="javascript: executeLink('prepareInsert.action?pcmat.obra.id=${obra.id}');"></button>
		<button class="btnVoltar" onclick="javascript: executeLink('list.action');"></button>
	</div>
