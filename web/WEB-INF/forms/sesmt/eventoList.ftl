...
<body>
	...
	<@display.table name="eventos" id="evento" class="dados">
		<@display.column title="Ações" class="acao">
			...
			<a href="#" onclick="if (confirm('Confirma exclusão?')) window.location='delete.action?evento.id=${evento.id}'"><img border="0" title="Excluir" src="<@ww.url value="/imgs/delete.gif"/>"></a>
		</@display.column>
