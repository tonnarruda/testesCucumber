	...
	<#if cargoTmp?exists && cargoTmp.id?exists>
		<@display.table name="funcaos" id="funcao" class="dados" sort="list">
			<@display.column title="Ações" class="acao">
				...
				<a href="#" onclick="if (confirm('Confirma exclusão?')) window.location='deleteFiltro.action?funcao.id=${funcao.id}&cargoTmp.id=${cargoTmp.id}&page=${page}'"><img border="0" title="Excluir" src="<@ww.url value="/imgs/delete.gif"/>"></a>
			</@display.column>
