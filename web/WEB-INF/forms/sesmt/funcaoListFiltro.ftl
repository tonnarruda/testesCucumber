	...
	<#if cargoTmp?exists && cargoTmp.id?exists>
		<@display.table name="funcaos" id="funcao" class="dados" sort="list">
			<@display.column title="Ações" class="acao">
				<a href="javascript:;" onclick="javascript: executeLink('prepareUpdateFiltro.action?funcao.id=${funcao.id}&cargoTmp.id=${cargoTmp.id}');"><img border="0" title="<@ww.text name="list.edit.hint"/>" src="<@ww.url value="/imgs/edit.gif"/>"></a>
				<a href="javascript:;" onclick="newConfirm('Confirma exclusão?', function(){executeLink('deleteFiltro.action?funcao.id=${funcao.id}&cargoTmp.id=${cargoTmp.id}&page=${page}');});"><img border="0" title="Excluir" src="<@ww.url value="/imgs/delete.gif"/>"></a>
			</@display.column>
		...
		<div class="buttonGroup">
			<button class="btnInserir" onclick="javascript: executeLink('prepareInsertFiltro.action?cargoTmp.id=${cargoTmp.id}');"></button>
		</div>
	</#if>
</body>
</html>