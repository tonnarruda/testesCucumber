...
<html>
	<head>
	<@ww.head/>
		<title>Matriz de Treinamento</title>
	...
	<body>
		...
		<@ww.form name="form" action="imprimirMatrizTreinamento.action" onsubmit="${validarCampos}" validate="true" method="POST">
			<@frt.checkListBox name="areasCheck" id="areasCheck" label="Áreas Organizacionais" list="areasCheckList" onClick="populaCargosByArea();"/>
			<@frt.checkListBox name="cargosCheck" id="cargosCheck" label="Cargos" list="cargosCheckList" onClick="populaFaixas();"/>
			<@frt.checkListBox name="faixaSalarialsCheck" id="faixaSalarialsCheck" label="Faixas Salariais (disponível se for selecionado apenas um Cargo)" list="faixaSalarialsCheckList" />
		</@ww.form>
