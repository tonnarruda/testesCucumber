...
<body>
	...
	<@display.table name="avaliacaoCursos" id="avaliacaoCurso" class="dados">
		<@display.column title="Ações" class="acao">
			...
			<a href="#" onclick="if (confirm('Confirma exclusão?')) window.location='delete.action?avaliacaoCurso.id=${avaliacaoCurso.id}'"><img border="0" title="Excluir" src="<@ww.url value="/imgs/delete.gif"/>"></a>
		</@display.column>
		...
		<@display.column property="minimoAprovacao" title="Mínimo para Aprovação" format="{0, number, #,##0.00}" style="width: 200px;text-align:right;"/>	
	</@display.table>
