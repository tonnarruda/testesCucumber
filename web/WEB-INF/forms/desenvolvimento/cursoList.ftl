...
<body>
	...
	<@display.table name="cursos" id="curso" class="dados">
		<@display.column title="Ações" class="acao" style = "width:95px";>
			<a href="javascript: executeLink('prepareUpdate.action?curso.id=${curso.id}&curso.empresa.id=${curso.empresa.id}');"><img border="0" title="<@ww.text name="list.edit.hint"/>" src="<@ww.url value="/imgs/edit.gif"/>"></a>
			<a href="javascript: executeLink('../turma/list.action?curso.id=${curso.id}');"><img border="0" title="Turmas" src="<@ww.url value="/imgs/db_add.gif"/>"></a>
			<a href="javascript: executeLink('../../geral/documentoAnexo/listCurso.action?documentoAnexo.origem=U&documentoAnexo.origemId=${curso.id}');"><img border="0" title="Inserir Anexos" src="<@ww.url includeParams="none" value="/imgs/anexos.gif"/>"></a>
			...
			<#if curso.empresa.id == empresaSistema.id>
				<a href="#" onclick="newConfirm('Confirma exclusão?', function(){executeLink('delete.action?curso.id=${curso.id}&curso.empresa.id=${curso.empresa.id}');});"><img border="0" title="Excluir" src="<@ww.url value="/imgs/delete.gif"/>"></a>
			<#else>
	...
	<div class="buttonGroup">
		<button class="btnInserir" onclick="executeLink('prepareInsert.action');" accesskey="N">
		</button>
	</div>
