...
<body>
	...
	<@display.table name="gastoEmpresas" id="gastoEmpresa" class="dados">
		<@display.column title="Ações" class="acao">
			...
			<a href="#" onclick="if (confirm('Confirma exclusão?')) window.location='delete.action?gastoEmpresa.id=${gastoEmpresa.id}&page=${page}'"><img border="0" title="Excluir" src="<@ww.url value="/imgs/delete.gif"/>"></a>
		</@display.column>
