...
<#else>
	...
	<@display.table name="colaboradorOcorrencias" id="colaboradorOcorrencia" class="dados">
		<@display.column title="Ações" class="acao">
			...
			<a href="#" onclick="if (confirm('Confirma exclusão?')) window.location='delete.action?colaboradorOcorrencia.id=${colaboradorOcorrencia.id}&colaborador.id=${colaborador.id}'"><img border="0" title="Excluir" src="<@ww.url value="/imgs/delete.gif"/>"></a>
		</@display.column>
