...
<body>
	...
	<@display.table name="grupoGastos" id="grupoGasto" class="dados">
		<@display.column title="Ações" class="acao">
			...
			<a href="#" onclick="if (confirm('Confirma exclusão?')) window.location='delete.action?grupoGasto.id=${grupoGasto.id}&page=${page}'"><img border="0" title="Excluir" src="<@ww.url value="/imgs/delete.gif"/>"></a>
		</@display.column>
