...
<@display.table name="empresaBdss" id="empresaBds" class="dados">
	<@display.column title="Ações" media="html" class="acao" style="text-align:center; width: 80px;">
		...
		<a href="#" onclick="if (confirm('Confirma exclusão?')) window.location='delete.action?empresaBds.id=${empresaBds.id}&page=${page}'"><img border="0" title="<@ww.text name="list.del.hint"/>" src="<@ww.url includeParams="none" value="/imgs/delete.gif"/>"></a>
	</@display.column>
