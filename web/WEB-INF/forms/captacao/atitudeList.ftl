...
<body>
	...
	<@display.table name="atitudes" id="atitude" class="dados">
		<@display.column title="Ações" media="html" style="text-align:center; width:80px;" >
			<a href="javascript:;" onclick="javascript: executeLink('prepareUpdate.action?atitude.id=${atitude.id}');"><img border="0" title="<@ww.text name="list.edit.hint"/>" src="<@ww.url value="/imgs/edit.gif"/>"></a>
			<a href="javascript:;" onclick="newConfirm('Confirma exclusão?', function(){executeLink('delete.action?atitude.id=${atitude.id}&page=${page}');});"><img border="0" title="<@ww.text name="list.del.hint"/>" src="<@ww.url value="/imgs/delete.gif"/>"></a>
		</@display.column>
