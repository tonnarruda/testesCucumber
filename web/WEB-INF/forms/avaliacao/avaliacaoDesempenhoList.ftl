	...
	<@display.table name="avaliacaoDesempenhos" id="avaliacaoDesempenho" class="dados">
		<@display.column title="Ações" class="acao" style="width:180px;">
			...
			<#if avaliacaoDesempenho.liberada>
				<a href="javascript:if (confirm('Deseja bloquear esta Avaliação?')) window.location='bloquear.action?avaliacaoDesempenho.id=${avaliacaoDesempenho.id}'"><img border="0" title="Bloquear" src="<@ww.url includeParams="none" value="/imgs/liberar.gif"/>"></a>
			...
			<#else>
				<a href="javascript:if (confirm('Deseja liberar esta Avaliação?')) window.location='liberar.action?avaliacaoDesempenho.id=${avaliacaoDesempenho.id}'"><img border="0" title="Liberar" src="<@ww.url includeParams="none" value="/imgs/bloquear.gif"/>"></a>
			...
			<a href="#" onclick="if (confirm('Confirma exclusão?')) window.location='delete.action?avaliacaoDesempenho.id=${avaliacaoDesempenho.id}'"><img border="0" title="Excluir" src="<@ww.url value="/imgs/delete.gif"/>"></a>
		</@display.column>
