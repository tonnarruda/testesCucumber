...
BEGIN 
     FOR mviews IN
		...
		LOOP
		RETURN QUERY
		    (select id, cast(nome as text) as nome, ativo from areaorganizacional where id = mviews.area_id ) union
		    ...
		    )SELECT id, cast(((select nome from areaorganizacional where id = mviews.area_id ) || ' > ' || nomeHierarquico) as text) as nome, ativo FROM areaorganizacional_recursiva) ORDER BY nome;
      END LOOP;
    RETURN; 
END; 
$$ LANGUAGE plpgsql; --.go