...
insert into papel (id, codigo, nome, url, ordem, menu, papelmae_id) values (687, 'ROLE_CONFIG_CAMPOS_PADROES_DO_SISTEMA','Campos padrões do sistema', '/geral/parametrosDoSistema/listCampos.action', 1, true, 503);--.go
insert into papel (id, codigo, nome, url, ordem, menu, papelmae_id) values (688, 'ROLE_CONFIG_CAMPOS_EXTRAS','Campos extras', '/geral/parametrosDoSistema/listCamposExtras.action', 2, true, 503);--.go
update papel set papelmae_id = 687 where id in(678,679,680);--.go
...
insert into papel (id, codigo, nome, url, ordem, menu, papelmae_id) values (689, 'ROLE_CONFIG_CAMPOS_EXTRAS_PARA_COLABORADOR','Cadastro de Colaborador', '#', 1, false, 688);--.go
insert into papel (id, codigo, nome, url, ordem, menu, papelmae_id) values (690, 'ROLE_CONFIG_CAMPOS_EXTRAS_PARA_CANDIDATO', 'Cadastro de Candidato', '#', 2, false, 688);--.go
insert into papel (id, codigo, nome, url, ordem, menu, papelmae_id) values (691, 'ROLE_CONFIG_CAMPOS_EXTRAS_PARA_CANDIDATO_EXT','Cadastro de Candidato (Externo)', '#', 3, false, 688);--.go
alter sequence papel_sequence restart with 692;--.go

insert into perfil_papel(perfil_id, papeis_id) select distinct perfil_id, 687 from perfil_papel where papeis_id in(678,679,680);--.go
insert into perfil_papel(perfil_id, papeis_id) select distinct perfil_id, 689 from perfil_papel where papeis_id in(678);--.go
insert into perfil_papel(perfil_id, papeis_id) select distinct perfil_id, 690 from perfil_papel where papeis_id in(679);--.go
insert into perfil_papel(perfil_id, papeis_id) select distinct perfil_id, 691 from perfil_papel where papeis_id in(680);--.go

insert into perfil_papel(perfil_id, papeis_id) select distinct perfil_id, 688 from perfil_papel where papeis_id in(689,690,691);--.go
...
select verifica_existencia_campo_extra();--.go
