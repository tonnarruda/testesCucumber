...
BEGIN 
	FOR mv IN 
		...
		when sub.status = 'P' then (
					with hist_promocao as ( select hc.id  as hist_id from historicocolaborador hc where hc.data = (select min(hc2.data) from historicocolaborador hc2 where hc2.estabelecimento_id = sub.estabelecimento_id 
											    and hc2.areaorganizacional_id = sub.areaorganizacional_id 
											    and hc2.data>=sub.data
						...
						then (select hist_promocao.hist_id from hist_promocao)
							...
							select case when(select exists (select hist_promocao2.hist_id from hist_promocao2) as promocao2)
								then (select hist_promocao2.hist_id from hist_promocao2)
								     ...
								     select hc.id from historicocolaborador hc 
											where hc.motivo = 'P' 
											and hc.colaborador_id = sub.colab_id
											and hc.data = (select min(hc2.data) from historicocolaborador hc2 
																where hc2.colaborador_id = sub.colab_id
																      and hc2.motivo = 'P' 
													)
								)
								end
		...
		when sub.status = 'I' or sub.status = 'A' then (
						with historico_id as(
							select hc.id as id from historicocolaborador hc where hc.data = (select min(hc2.data) from historicocolaborador hc2 where 
															hc2.estabelecimento_id = sub.estabelecimento_id 
															and hc2.areaorganizacional_id = sub.areaorganizacional_id 
															and hc2.data>=sub.data
						...
						select case when ( select exists (select  historico_id.id from historico_id) as t)
							then (select historico_id.id from historico_id)
								...
								with historico_id2 as( select hc.id as id2 from historicocolaborador as hc 
														where hc.colaborador_id = sub.colab_id
														      and hc.data = (select min(hc2.data) from historicocolaborador hc2 
																				where hc2.data >= sub.data 
																				and hc2.colaborador_id = sub.colab_id
																     )
							...
							select case when (select exists (select historico_id2.id2 from historico_id2) as historico)
							       then (select historico_id2.id2 from historico_id2)
									...
									select hc4.id from historicocolaborador hc4 where hc4.motivo = 'C' and hc4.colaborador_id = sub.colab_id
								)
							       end
							)
							end 
		...
		select c.solicitacao_id as sol_id, c.candidato_id as cand_id, c.id as colab_id, cs.status, cs.id, 
