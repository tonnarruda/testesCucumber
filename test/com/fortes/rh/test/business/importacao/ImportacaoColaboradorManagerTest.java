...
public class ImportacaoColaboradorManagerTest extends MockObjectTestCase {
	...
	public void testImportarDadosPessoaisByCpf() throws IOException
    {
    	...
    	colaboradorManager.expects(once()).method("findByCpf").with(eq("26745534304"), eq(1L)).will(returnValue(Arrays.asList(colaborador)));
    	colaboradorManager.expects(once()).method("findByCpf").with(eq("52986063420"), eq(1L)).will(returnValue(null));
