...
public class GastoEmpresaManagerTest extends MockObjectTestCase
{
	...
	public void testFiltroRelatorioByArea() throws Exception
	{
		...
		areaOrganizacionalManager.expects(once()).method("findAllList").with(ANYTHING, ANYTHING).will(returnValue(new ArrayList<AreaOrganizacional>()));
	...
	public void testFiltroRelatorioByColaborador() throws Exception
	{
		...
		areaOrganizacionalManager.expects(atLeastOnce()).method("findAllList").with(eq(1L),eq(AreaOrganizacional.TODAS)).will(returnValue(areaOrganizacionals));
