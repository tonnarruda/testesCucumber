...
public class HistoricoCandidatoDWRTest extends MockObjectTestCase
{
	...
	public void testGetCandidatoAptoByEtapa()
	{
		...
		candidatoSolicitacaoManager.expects(once()).method("getCandidatoSolicitacaoList").with(new Constraint[] {ANYTHING, ANYTHING, eq(solicitacao.getId()), eq(etapaSeletiva.getId()), ANYTHING, eq(true), eq(true), eq(true), ANYTHING, ANYTHING}).will(returnValue(candidatoSolicitacaos));
