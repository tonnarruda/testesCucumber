...
public class ColaboradorQuestionarioManagerImpl extends GenericManagerImpl<ColaboradorQuestionario, ColaboradorQuestionarioDao> implements ColaboradorQuestionarioManager
{
	...
	private void validaAssociacao(Collection<Colaborador> avaliados, Collection<Colaborador> avaliadores, boolean permiteAutoAvaliacao) throws Exception 
	{
		if (avaliados.isEmpty() || avaliadores.isEmpty())
			throw new FortesException("Não foi possível liberar esta avaliação: número insuficiente de participantes.");
	...
	public ColaboradorQuestionario findByColaboradorAndAvaliacaoDesempenho(Long avaliadoId, Long avaliacaoDesempenhoId) {
		return getDao().findByColaboradorAndAvaliacaoDesempenho(avaliadoId, avaliacaoDesempenhoId);
	}
