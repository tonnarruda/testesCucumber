...
/*public String insertCandidatoExterno() throws Exception
{
	if ("".equals(getSenha()))
	{
	...
	else if (getSenha().equals(getConfirmaSenha()))
	{
		candidatoManager.save(candidato);
		return Action.SUCCESS;
	}
	...
	{
		addFieldError("senha", "Senhas não são iguais.");
		return Action.INPUT;
	}

}*/

/*public String prepareUpdateCandidatoExterno() throws Exception
{
	...
	setCpf((String) session.getAttribute("SESSION_CPF"));
	candidato = candidatoManager.findByCPF(getCpf());

	return Action.SUCCESS;
}*/


// Removido daqui e levada para o list para ser usado na inclusão dos candidatos a solicitação
/*public String prepareBuscaCandidato() throws Exception
{
...
{
	if ("exportar".equals(getTipoAcao()))
		exportaCandidatos();
...
public void buscaCandidato() throws Exception
{

	candidatosAchados = candidatoManager.findBusca(getNomeBusca(),
			getSexoBusca(), getIdadeMinBusca(), getIdadeMaxBusca(),
			getCpfBusca(), getCidadeBusca(), getUfBusca(),
			getEscolaridadeBusca(), getAreasInteresse());

	if (candidatosAchados.size() == 0)
		candidatosAchados = null;
}*/

/*private void exportaCandidatos() throws Exception
{
	if (selectedIds != null && selectedIds.length > 0)
	{
		candidatosExporta = candidatoManager.findByIds(selectedIds);

		//A implementação de Exportação não deve ficar no manager???
		String encoding = "UTF-8";
		XStream stream = new XStream(new DomDriver(encoding));
		//verificar se o arquivo é gerado corretamente caso dois ou
		// mais usuários mandem importar ao mesmo tempo.
		File xmlFile = new File(".\\candidatos.xml");

		String xmlFileContent = new String(
				"<?xml version=\"1.0\" encoding=\"UTF-8\" ?>\r\n"
						.getBytes(), encoding);
		xmlFileContent += stream.toXML(candidatosExporta);
		...
		ZipOutputStream zipOutputStream = new Zip().compress(
				new File[] { xmlFile }, "candidatos");// cria o arquivo
														// candidatos.zip
		zipOutputStream.close();

		File zipFile = new File("candidatos.zip");
		...
		try
		{
			new Mail()
					.send(
							...
							"rh",
							"",
							"Re: Solicitação de Banco de Dados Solidário",
							destinatario,
							"Rhesus",
							"Passos para a Importação dos Candidatos:\r\n\t 1. Descompacte o arquivo anexo;\r\n\t 2. Acesse Talentos > [Captação de Talentos]Banco de Dados Condidato; \r\n\t 3. Clique em \"Importar Candidatos\", depois em \"Procurar\" e selecione o arquivo que acabou de ser descompactado.Em seguida clique em \"Enviar\"",
							"candidatos", new File[] { zipFile });
			msgExportacao = "Exportação realizada com sucesso.";
		}
		catch (MessagingException e)
		{
			msgExportacao = "Não foi possível enviar o arquivo de Exportação.";

			if (e instanceof SendFailedException)
				msgErroExportacao = "Email inválido";
			else
				msgErroExportacao = "Verifique sua conexão com o Servidor de Email.";
		}
		catch (Exception e)
		{
			msgExportacao = "Não foi possível enviar o arquivo de Exportação.";
			msgErroExportacao = "Arquivo com problema.";
		}
		finally
		{
			zipFile.delete();
