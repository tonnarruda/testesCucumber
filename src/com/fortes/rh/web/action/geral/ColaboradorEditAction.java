	...
	public String preparePerformanceFuncional() throws Exception
	{
		if (colaborador != null && colaborador.getId() != null)
		{
			...
			if(SecurityUtil.verifyRole(ActionContext.getContext().getSession(), new String[]{"ROLE_VISUALIZAR_ANEXO_COLAB_LOGADO"}))
				documentoAnexosColaborador = documentoAnexoManager.getDocumentoAnexoByOrigemId(OrigemAnexo.COLABORADOR, colaborador.getId());

			if(colaborador.getCandidato() != null && colaborador.getCandidato().getId() != null)
			{
				if(SecurityUtil.verifyRole(ActionContext.getContext().getSession(), new String[]{"ROLE_VISUALIZAR_ANEXO_COLAB_LOGADO"}))
					documentoAnexosCandidato = documentoAnexoManager.getDocumentoAnexoByOrigemId(OrigemAnexo.CANDIDATO, colaborador.getCandidato().getId());
