# language: pt

Funcionalidade: Indicador Estatísticas de Vagas por Motivo

  Cenário: Indicador Estatísticas de Vagas por Motivo
    Dado que eu esteja logado
    Dado que exista o estabelecimento "centro"
    Dado que exista a área organizacional "geral"
    Quando eu acesso o menu "R&S > Indicadores > Estatísticas de Vagas por Motivo"
    Então eu devo ver o título "Recrutamento e Seleção - Estatísticas de Vagas por Motivo"
    E eu clico no botão "Relatorio"
    Então eu devo ver o alert do valida campos e clico no ok
    E eu preencho o campo (JS) "dataDe" com "02/02/2000"
    E eu preencho o campo (JS) "dataAte" com "05/05/2000"
    E eu marco "centro"
    E eu marco "geral"
    E eu clico no botão "Relatorio"
    Entao eu devo ver "Não existem dados para o filtro informado."